...
== 評価(eval)
Lisp(List Proseccing language)の世界では, 
その名の通り, リストというデータ構造を基本としてる. 
リストの各要素はアトムと呼ばれる.  

Lisp で関数を呼び出す式は次のような形になる. 
 > (関数名　引数1 引数2)
このような形の式をS式という. このリストをS式として
Lispに解釈させることを「評価(eval)」という. 
S式として評価させたくない場合には’ を使って
以下のように書く. 
 > '(atom1 atom2 atom3)
 (ATOM1 ATOM2 ATOME3)
